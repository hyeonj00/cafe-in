<?php
    ...
    // session_start();
    // if (isset($_SESSION["userid"])) $userid = $_SESSION["mem_id"];
    // else $userid = "";
	// $userid = "test";

    // 쿠폰 정보 조회 
    $query = "SELECT num, own_id, mem_id, end_day FROM coupon WHERE mem_id = '$userid'";
    ...
    <div class="wrap">
        ...
        <div id="m_content" class="m_content">
            <div class="coupon_div">
                <?php
                    // Iterate over the coupon results
                    while ($row = mysqli_fetch_assoc($result)) {
                        $ownId = $row['own_id'];

                        // Retrieve cafe_info for the corresponding own_id
                        $cafeInfoQuery = "SELECT coupon, coupon_detail FROM cafe_info WHERE own_id = '$ownId'";
                        $cafeInfoResult = mysqli_query($conn, $cafeInfoQuery);
                        $cafeInfoRow = mysqli_fetch_assoc($cafeInfoResult);
                        $couponImagePath = $cafeInfoRow['coupon'];
                        $couponDetailImagePath = $cafeInfoRow['coupon_detail'];
                ?>
                        <div id="gridd">
                            <div class="flip-box">
                                <div class="flip-box-inner">
                                    <div class="flip-box-front">
                                        <img src="img/coupon/<?php echo $couponImagePath; ?>" width="100%" height="100%">
                                    </div>
                                    <div class="flip-box-back">
                                        <img src="img/coupon/<?php echo $couponDetailImagePath; ?>" width="100%" height="100%">
                                    </div>
                                </div>
                            </div>
                        </div>
