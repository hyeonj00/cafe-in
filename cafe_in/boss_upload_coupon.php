...
if (move_uploaded_file($_FILES["image1"]["tmp_name"], $target_file1)) {
   echo "<span style='font-size: larger; font-weight: bold;'>쿠폰 이미지 업로드를 완료하였습니다. </span>";
} else {
   echo "<span style='font-size: larger; font-weight: bold;'>죄송합니다.<br> 쿠폰 이미지를 업로드하는 중에 오류가 발생했습니다.</span><br><br>";
}

// 두 번째 이미지 업로드
if (move_uploaded_file($_FILES["image2"]["tmp_name"], $target_file2)) {
   echo "<span style='font-size: larger; font-weight: bold;'>쿠폰 설명 이미지 업로드를 완료하였습니다. </span>";
} else {
   echo "<span style='font-size: larger; font-weight: bold;'>죄송합니다.<br> 쿠폰 설명 이미지를 업로드하는 중에 오류가 발생했습니다.</span>";
}
