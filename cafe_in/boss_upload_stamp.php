	...
	<div class="wrap">
		...
		<div id="m_content" class="m_content">
			...
			<div class="profile_div">
				...
				// 이미지 업로드 대상 디렉토리 설정
				$target_dir = "/workspace/cafe_in/img/stamp/";
				...
				if (move_uploaded_file($_FILES["image1"]["tmp_name"], $target_file1)) {
					echo "<span style='font-size: larger; font-weight: bold;'>스탬프 이미지를 수정하였습니다.</span><br>";
				} else {
					echo "<span style='font-size: larger; font-weight: bold;'>죄송합니다.<br> 스탬프 이미지를 업로드하는 중에 오류가 발생했습니다.</span><br><br>";
				}

				// 두 번째 이미지 업로드
				if (move_uploaded_file($_FILES["image2"]["tmp_name"], $target_file2)) {
					echo "<span style='font-size: larger; font-weight: bold;'>스탬프북 이미지를 수정하였습니다.</span><br>";
				} else {
					echo "<span style='font-size: larger; font-weight: bold;'>죄송합니다.<br> 스탬프북 이미지를 업로드하는 중에 오류가 발생했습니다.</span><br>";
				}
				...
				// 데이터베이스에 이미지 이름 삽입
				$stmt = $db->prepare("UPDATE cafe_info SET stamp=?, stampbook=? WHERE own_id=?");
